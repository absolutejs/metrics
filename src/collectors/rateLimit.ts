...
export const rateLimitCollector =
	...
	async () => {
		...
		if (m.allow !== undefined) {
			samples.push(
				counter('abs_rate_limit_decisions_total', m.allow, {
		...
		if (m.block !== undefined) {
			samples.push(
				counter('abs_rate_limit_decisions_total', m.block, {
					help: 'Rate-limit decisions',
		...
		if (m.drift !== undefined) {
			samples.push(
				counter('abs_rate_limit_drift_total', m.drift, {
		...
		if (m.rps !== undefined) {
			samples.push(
				gauge('abs_rate_limit_rps', m.rps, {
		...
		if (m.store?.entries !== undefined) {
			samples.push(
				gauge('abs_rate_limit_store_entries', m.store.entries, {
		...
		if (m.store?.capacity !== undefined) {
			samples.push(
				gauge('abs_rate_limit_store_capacity', m.store.capacity, {
